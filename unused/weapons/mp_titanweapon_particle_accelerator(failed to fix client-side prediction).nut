...
void function UpdateSplitterRifleWeaponMods( entity weapon )
{
	if ( !weapon.IsWeaponInAds() )
		RemoveSplitterRifleADSMods( weapon )
	else
		AddSplitterRifleADSMod( weapon )

	// some failsafe case: remove ADS mod if we don't have enough energy... for better prediction
	// welp it can work bad because SetMod() updates weapon's firing loop sound
...
void function AddSplitterRifleADSMod( entity weapon )
{
	array<string> mods = weapon.GetMods()
	int modsCount = mods.len()

	// FD case
	if ( weapon.HasMod( "fd_split_shot_cost") )
	{
		if ( weapon.HasMod( "pas_ion_weapon_ads" ) && !weapon.HasMod( "fd_upgraded_proto_particle_accelerator_pas" ) )
			mods.append( "fd_upgraded_proto_particle_accelerator_pas" )
		else if ( !weapon.HasMod( "fd_upgraded_proto_particle_accelerator" ) )
			mods.append( "fd_upgraded_proto_particle_accelerator" )
	}
	else // default case
	{
		if ( weapon.HasMod( "pas_ion_weapon_ads" ) && !weapon.HasMod( "proto_particle_accelerator_pas" ) )
			mods.append( "proto_particle_accelerator_pas" )
		else if ( !weapon.HasMod( "proto_particle_accelerator" ) )
			mods.append( "proto_particle_accelerator" )
	}

	bool modsUpdated = modsCount != mods.len()
	if ( modsUpdated )
		weapon.SetMods( mods )
}

void function RemoveSplitterRifleADSMods( entity weapon )
{
	array<string> mods = weapon.GetMods()
	int modsCount = mods.len()

	if ( weapon.HasMod( "proto_particle_accelerator" ) )
		mods.fastremovebyvalue( "proto_particle_accelerator" )
	if ( weapon.HasMod( "proto_particle_accelerator_pas" ) )
...
function FireWeaponPlayerAndNPC( entity weapon, WeaponPrimaryAttackParams attackParams, bool playerFired )
{
	...
	#if CLIENT
		// debug
		//print( "Calling FireWeaponPlayerAndNPC() on client!" )
		if ( !playerFired )
	...
	if ( shouldCreateProjectile )
	{
		...
		// try to rework stuffs here...
		// welp this won't help, raise from offhand with ads state WILL reset mods and won't be applied once again
		//weapon.ResetWeaponToDefaultEnergyCost()
		//int cost = weapon.GetWeaponCurrentEnergyCost()
		int currentEnergy = owner.GetSharedEnergyCount()
		//bool outOfEnergy = (currentEnergy < cost) || (currentEnergy == 0)
		// weapon mods think
		UpdateSplitterRifleWeaponMods( weapon )
		int cost = weapon.GetWeaponSettingInt( eWeaponVar.shared_energy_cost )
		bool outOfEnergy = (currentEnergy < cost) || (currentEnergy == 0)
		...
		// debug
		#if CLIENT
		//	print( "cost: " + string( cost ) )
		//	print( "shotCount: " + string( shotCount ) )
		#endif
	...
	//return 1 // don't do any return value, or 
}
