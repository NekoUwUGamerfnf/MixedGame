...
void function OnWeaponActivate_titanweapon_xo16( entity weapon )
{
	...
	if( owner.IsTitan() )
	{
		entity coreweapon = owner.GetOffhandWeapon( OFFHAND_EQUIPMENT )
		if( weapon.HasMod( "burst_core" ) && coreweapon.HasMod( "burst_core" ) )
			( weapon )
	}
	*/
}
...
var function OnWeaponPrimaryAttack_titanweapon_xo16( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if( weapon.HasMod( "burst_core" ) && coreweapon.HasMod( "burst_core" ) )
		FireCoreWeaponPlayer( weapon, attackParams )
	else
		weapon.FireWeaponBullet( attackParams.pos, attackParams.dir, 1, damageTypes.largeCaliber | weapon.GetWeaponDamageFlags() )
...
int function FireCoreWeaponPlayer( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	entity owner = weapon.GetWeaponOwner()
	...
	if ( attackParams.burstIndex == 0 )
	{
		...
		#endif
		OnAbilityStart_TitanCore( coreweapon )
	}
