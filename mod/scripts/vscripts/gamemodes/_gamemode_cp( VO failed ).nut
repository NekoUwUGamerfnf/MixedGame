...
void function SpawnHardpoints()
{
	foreach ( entity spawnpoint in GetEntArrayByClass_Expensive( "info_hardpoint" ) )
	{
		...
		thread GamemodeCP_VO_Think( hardpointStruct.hardpoint )
...
{
	hardpoint.oldState = state // set oldState before state actually changes
	SetGlobalNetInt( "objective" + GetHardpointGroup( hardpoint.hardpoint ) + "State", state )
	hardpoint.hardpoint.SetHardpointState( state )
	SignalHardpointStateForVO( hardpoint )
}

void function SignalHardpointStateForVO( HardpointStruct hardpoint )
{
	table result // table for GamemodeCP_VO_Think()
	result.oldState  <- hardpoint.oldState
	result.oldTeam  <- hardpoint.oldTeam
	result.newState  <- GetHardpointState( hardpoint )
	result.newTeam  <- hardpoint.hardpoint.GetTeam()
	hardpoint.hardpoint.Signal( "CapturePointStateChange", result )
}
...
{
	hardpoint.oldTeam = hardpoint.hardpoint.GetTeam() // set oldTeam here
	...
	while ( GamePlayingOrSuddenDeath() )
	{
		...
		else if( cappingTeam == TEAM_UNASSIGNED ) // nobody on point
		{
			if( ( GetHardpointState( hardpoint ) == CAPTURE_POINT_STATE_AMPED ) || ( GetHardpointState( hardpoint ) == CAPTURE_POINT_STATE_AMPING ) )
			{
				...
				if( GetHardpointState( hardpoint ) != CAPTURE_POINT_STATE_SELF_UNAMPING )
				{
