...
global function ArcCannon_HideIdleEffect
	...
	global function Arc_ConvertTitanShieldIntoBonusCharge
#endif
...
global const ARC_CANNON_TITAN_RANGE_CHAIN			= 750//900		// Max distance we can arc from one target to another
global const ARC_CANNON_CHAIN_COUNT_MIN			= 1			// Max number of chains at no charge
global const ARC_CANNON_CHAIN_COUNT_MAX			= 1			// Max number of chains at full charge
global const ARC_CANNON_CHAIN_COUNT_NPC			= 1			// Number of chains when an NPC fires the weapon
...
global const ARC_CANNON_RANGE_CHAIN_BURN			= 500
global const ARC_CANNON_TITAN_RANGE_CHAIN_BURN		= 700
global const ARC_CANNON_CHAIN_COUNT_MIN_BURN		= 2		// Max number of chains at no charge
global const ARC_CANNON_CHAIN_COUNT_MAX_BURN		= 2		// Max number of chains at full charge
global const ARC_CANNON_CHAIN_COUNT_NPC_BURN		= 2		// Number of chains when an NPC fires the weapon
global const ARC_CANNON_FORK_COUNT_MAX_BURN		= 2		// Number of forks that can come out of one target to other targets
global const ARC_CANNON_BEAM_LIFETIME_BURN			= 0.75
...
// Damage
global const ARC_CANNON_DAMAGE_FALLOFF_SCALER		= 0.50		// Amount of damage carried on to the next target in the chain lightning. If 0.75, then a target that would normally take 100 damage will take 75 damage if they are one chain deep, or 56 damage if 2 levels deep
global const ARC_CANNON_DAMAGE_CHARGE_RATIO		= 0.85		// What amount of charge is required for full damage.
global const ARC_CANNON_DAMAGE_CHARGE_RATIO_BURN	= 0.85		// What amount of charge is required for full damage.
global const ARC_CANNON_CAPACITOR_CHARGE_RATIO		= 1.0

// Options
global const ARC_CANNON_TARGETS_MISSILES 			= 0			// 1 = arc cannon zaps missiles that are active, 0 = missiles are ignored by arc cannon

//Mods
global const ARC_OVERCHARGE_MAX_SHIELD_DECAY       	= 0.2
global const ARC_OVERCHARGE_SHIELD_DECAY_MULTIPLIER 	= 0.04
global const ARC_OVERCHARGE_BONUS_CHARGE_FRACTION		= 0.05

global const ARC_SPLITTER_DAMAGE_FALLOFF_SCALER		= 0.6
global const ARC_SPLITTER_FORK_COUNT_MAX				= 10
...
global const ARC_CANNON_BEAM_EFFECT_MOD = $"wpn_arc_cannon_beam"
...
function ArcCannon_Init()
{
	...
	#if CLIENT
		AddDestroyCallback( "mp_titanweapon_sniper", ClientDestroyCallback_ArcCannon_Stop )
	#else
	...
	//thread MonitorChargeFraction(weapon)
		...
		if ( weapon.HasMod( "overcharge" ) )
		{
			if ( weaponOwner.IsTitan() )
			{
				...
				thread Arc_ConvertTitanShieldIntoBonusCharge( soul, weapon )
			}
		}
	#endif
...
#if SERVER
function Arc_ConvertTitanShieldIntoBonusCharge( entity soul, entity weapon )
{
	weapon.EndSignal( ARC_CANNON_SIGNAL_CHARGEEND )
	weapon.EndSignal( "OnDestroy" )

	local maxShieldDecay = ARC_OVERCHARGE_MAX_SHIELD_DECAY
	local bonusChargeFraction = ARC_OVERCHARGE_BONUS_CHARGE_FRACTION
	local shieldDecayMultiplier = ARC_OVERCHARGE_SHIELD_DECAY_MULTIPLIER
	...
	if ( vortexHit )
	{
		expect VortexBulletHit( vortexHit )
			...
			else if ( IsVortexSphere( vortexHit.vortex ) )
			{
				...
				local damage = 120 * GraphCapped( chargeFrac, 0, 1, 0.0, 10.0 ) // do more damage the more charged the weapon is.
	...
	thread CreateArcCannonBeam( zapInfo.weapon, target, beamStartPos, beamEndPos, zapInfo.player, ARC_CANNON_BEAM_LIFETIME, zapInfo.radius, boltWidth, 5, true, firstBeam )
		...
		if ( IsValid( target ) && IsValid( zapInfo.player ) )
		{
			...
			if ( IsValid( zapInfo.weapon ) && zapInfo.weapon.HasMod( "splitter" ) )
				damageFalloff = ARC_SPLITTER_DAMAGE_FALLOFF_SCALER
...
array<entity> function GetArcCannonChainTargets( vector fromOrigin, entity fromTarget, table zapInfo )
{
	...
	foreach ( ent in allTargets )
	{
		...
		if ( zapInfo.weapon.HasMod( "splitter" ) )
			forkCount = ARC_SPLITTER_FORK_COUNT_MAX
...
array<entity> function GetArcCannonTargets( vector origin, int team )
{
	...
	if ( ARC_CANNON_TARGETS_MISSILES && file.canDestroyMissiles == true)
...
array<entity> function GetArcCannonTargetsInRange( vector origin, int team, entity weapon )
{
	...
	foreach( target in allTargets )
	{
		if(target.GetTargetName() == "Arc Ball" && target.GetOwner() == weapon.GetWeaponOwner())
		{
			...
			titanDistSq = ARC_CANNON_TITAN_RANGE_CHAIN * ARC_CANNON_TITAN_RANGE_CHAIN * 1.5
			distSq = ARC_CANNON_RANGE_CHAIN * ARC_CANNON_RANGE_CHAIN * 1.5
		}
